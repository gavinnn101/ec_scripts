...
public class ChopWillowTreesBranch extends Branch {
    ...
    public boolean isValid() {
        for (Map.Entry<Skill, LevelRange> entry : Woodcutting.WILLOW_TREE_LEVEL_RANGES.entrySet()) {
            ...
            if (currentLevel >= range.getMin() && currentLevel < range.getMax()) {
                Main.setActivity("Woodcutting willow trees to lv41", Skill.WOODCUTTING, range.getMax());
                return true;
            }
        }
        return false;
