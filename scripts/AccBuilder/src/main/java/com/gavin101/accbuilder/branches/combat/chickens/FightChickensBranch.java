...
import static com.gavin101.accbuilder.constants.combat.ChickenCombat.CHICKEN_LEVEL_RANGES;
...
public class FightChickensBranch extends Branch {
    ...
    public boolean isValid() {
        if (!General.canStartCombatTier(CHICKEN_LEVEL_RANGES)) {
            return false;
        }

        for (Map.Entry<Skill, LevelRange> entry : CHICKEN_LEVEL_RANGES.entrySet()) {
            ...
            if (currentLevel >= range.getMin() && currentLevel < range.getMax()) {
                Main.setActivity("Training " + skill + " on chickens", skill, range.getMax());
